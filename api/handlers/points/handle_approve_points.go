...
func (c *PointsController) handleApprovePoints(ctx context.Context, req *approvePointsRequest) (approvePointsResponse, error) {
	...
	if !hasAccess {
		...
		return resp, fmt.Errorf("requesting user does not have permission to user's records")
	}
	...
	if point.UserID != req.ChildID {
		return resp, fmt.Errorf("point user id %v does not match request %v", point.UserID, req.ChildID)
	}
